...
//                }
    ...
    private fun checkRole(credentials: Credentials): Result<CheckRoleResult> {
        ...
        if (roleResponse.isSuccessful) {
            val responseJson = roleResponse.body?.string() ?: return Result.failure(Exception("Errore nell'autorizzazione"))
            println(responseJson)
            result.peerReviewRoleResponse = Json.decodeFromString(responseJson)

            className = result.peerReviewRoleResponse?.class_name ?: return Result.failure(Exception("Errore nell'autorizzazione"))
...
//
//        val enroll = PeerReviewEnrollJsonData(
//            full_name = name,
...
//        )
//
//        val enrollResponse = getEnroll(enroll, client)
//        return if (enrollResponse.status.isSuccess()) {
//            println("Iscrizione avvenuta con successo!")
//            val courseID = enrollResponse.bodyAsText()
//            println("CourseID: $courseID")
//            credentials.courseID = courseID
//            true
//        } else {
//            println("Errore nell'iscrizione al corso")
//            false
//        }
//    }


//    private suspend fun getEnroll(enrollData: PeerReviewEnrollJsonData, client: HttpClient): HttpResponse {
//        return client.post(ApiHelper.postEnroll()) {
